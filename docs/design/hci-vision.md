...
### The radio lives between races

Race weekends are rare — a fan gets maybe eight a year. A product that only matters sixteen days a year dies: no development loop, no community, no reason to carry it. So racing is the *wedge* — the identity, the RE disruption — but the radio is an **event scanner for every day**:

- **Weather and broadcast are daily keys, already in the design** — long-5 (NOAA) and long-0 (FM) are label-honest hardware. A fan checking a thunderstorm line on a Tuesday is using the same radio they'll hold at the 500.
- **Local short-track racing is weekly** — dirt bullrings run every Friday and Saturday night, spring through fall, and the WoO data (151–160 MHz VHF tracks) is the seed of that library. The pack model applies to a Saturday night at Putnamville exactly as it does to the Indy 500.
- **CAPTURE is a discovery tool, not a race tool** — it catches *any* signal from the air: an air show, a marathon, a high-school football coach, the aircraft overhead, the neighbor's FRS. One long-PTT press, saved, named, scanned. That is a daily activity, and it feeds the community database daily — not just on race weekends.
- **The HCI already generalized itself:** the identity is a string ("24", "WX", "TWR", "MAR16"), the 36-glyph font renders it, and WX/BRD are printed labels. The same three states serve the Indy 500 and a Tuesday afternoon. No new UI.
- **A demo pack ships everywhere** (app default + flash fallback, spec P1 §8): airband guard/unicom, marine, FRS — a new owner scans something useful the first evening, no race required. A radio that boots into "NO PACK" is a radio that gets returned.
...
- **The library and the desktop app:** the full frequency library — every series' stock channels (WoO raceceiver, MRN, officials), every track's frequencies — lives in `race-packs/library/` and in the packtool app, **never in the radio**. The radio holds one weekend's subset (≤ 64 cars / 24 stations, spec §4.3); the app assembles it: pick the races you're attending (Brickyard + IRP, Daytona 24 + the local short track), it merges event packs with library stations, dedupes, and reports the capacity trade-off when you've picked more than fits. "How many stations can it hold?" is the wrong question — the right one is "what are you listening to this weekend?"
- **Daily presets are library data too** (`race-packs/library/daily/`): airband, marine, FRS — the demo pack's source, composed by the app into a default everyday pack. Capture feeds the library every day, not just on race weekends: a caught aircraft frequency is a contribution like a caught race channel.
...
- **P2 — Scan engine + data — SHIPPED (2026-08):** the scan engine (tone-lock landing, FOLLOW, the CSQ guard, group filters), the packtool (CLI: validate/build/diff/import/compose/dump/flash — the "desktop app" vision landed as a toolchain first; a GUI can wrap it), open `race-packs/` v1 with the IMS 2026 draft + the daily library. *Remaining: the (hw) gates — real-glass screenshots, the bench-rig two-radio tests (docs/bench-rig.md), the fan hand-off — and P0 field data (Indy 500 May 2026, IRP Saturday nights). The community-verified-pack gate stands for v1 of the data.*
