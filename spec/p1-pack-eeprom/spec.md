...
- `PACK_SaveLockout(i)`, `PACK_SaveFavorite(i)`, `PACK_SetMyDriver()`, `PACK_AddCapture(entry)` — each writes the minimal region + CRC. **The pack layer is the single mutation choke point: in PRACTICE mode (`gPracticeMode`, RAM) all mutations are RAM-only stubs (dropped on power-off); in RACE with the seal flag set they are refused.** Boot always returns to RACE.
- `PACK_Load()` also restores `gPracticeMode = false` and the seal bit from the header flags byte; `PACK_IsSealed()` feeds the UI status line. **Exiting PRACTICE (long-EXIT = HOME, or power cycle) discards the practice session — the pack is untouched.**
- Boot screen (vision §5.8) renders series/track/session + car count from the header.
- Band-lock: a single filter function `PACK_FreqAllowed(freq, mode)` used by every tune path (BK4819 set, CAPTURE, BRD, WX) — single point of enforcement. **The valid set is the entry bands (§3/§6), in both modes:** 450–470 / 162.4–162.55 / 151–160 / **108–137 AM (airband)** / **156–162 (marine VHF)** / **462.55–467.725 (FRS/GMRS)** / 88–108 broadcast — a daily pack legitimately contains airband/marine/FRS entries. **PRACTICE additionally permits frequency entry/scanning across that set plus 2m 144–148** (receive-only, legal). **Cellular (824–894, 1710–1755, 1850–1990, 2110–2155) is hard-rejected in both modes, always (ECPA §2511).**
